...
io.on("connection", socket => {
  ...
  // WebRTC signaling for one-on-one DM calls.
  // Supports the restored client payload shape: { chatId, targetId, offer/answer/candidate }.
  function relayCall(event, payload) {
    ...
    const allowed = userCanAccessChat(uid, chatId) && userCanAccessChat(targetId, chatId);
    if (!["decline", "end"].includes(event) && !allowed) return;

    const out = { chatId, scope: "chat", scopeId: chatId, from: publicUser(uid), fromUserId: uid };
    ...
    if (typeof payload.speaking !== "undefined") out.speaking = !!payload.speaking;
  ...
  ["invite","accept","decline","offer","answer","ice","end","mute","speaking"].forEach(name => {
